...
import { updateVpaEquipment } from "../../redux/actions/vpaInventoryAction";
function EditVpaModal({ match, history }) {
  ...
  useEffect(() => {
    if (vpas) {
      setEquipmentData({ ...vpas });
    }
  }, [vpas]);
  useEffect(() => {
    dispatch(updateVpaEquipment(equipmentData, vpaId));
  }, [dispatch, equipmentData, vpaId]);
