...
import { updateIctEquipment } from "../../redux/actions/ictInventoryAction";
function EditIctModal({ match, history }) {
  ...
  useEffect(() => {
    if (icts) {
      setEquipmentData({ ...icts });
    }
  }, [icts]);
  useEffect(() => {
    dispatch(updateIctEquipment(equipmentData, ictId));
  }, [dispatch, equipmentData, ictId]);
