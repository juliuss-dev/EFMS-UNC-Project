...
import { updateMaintenanceEquipment } from "../../../redux/actions/inventoryAction";
function EditMaintenanceEquipment({ match, history }) {
  ...
  useEffect(() => {
    if (inventorys) {
      setEquipmentData({ ...inventorys });
    }
  }, [inventorys]);
  useEffect(() => {
    dispatch(updateMaintenanceEquipment(equipmentData, inventoryId));
  }, [dispatch, equipmentData, inventoryId]);
