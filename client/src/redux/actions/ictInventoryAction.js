...
export const getAllIctPrinter = () => async (dispatch) => {
  try {
    dispatch({ type: START_LOADING });
    const response = await axios.get(
      "/api/ictDepartmentInventory/getAllIctPrinter"
    );
    console.log(response);
    // alert(response.data.getUnits.sum_units);
    dispatch({ type: STOP_LOADING });
    dispatch({ type: GET_ALLDESKTOP, payload: response.data.getAllIctPrinter });
  } catch (error) {
